...
from Final2 import Ui_FinalWindow
...
class Ui_SoundEquilizer(object):
    ...
    def RectWind(self):
        ...
        for i in range (len(self.Magnitude)):
            ...
            self.output2.append(complex(tx,ty))

        self.output2=ifft(self.output2)
        self.output2=np.real(self.output2)
    ...
    def ApplyChanges(self,SoundEquilizer):
        if(self.RectWindow.isChecked()):
            self.RectWind()
            self.CompareOutputs(SoundEquilizer)
    ...
    def CompareOutputs(self , SoundEquilizer):
        self.window=QtWidgets.QMainWindow()
        self.ui=Ui_FinalWindow(self.data,self.output1,self.output2,self.rate)
